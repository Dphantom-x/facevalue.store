/* ============================================================
   FaceValue — Fan: verify (World ID) → agent buys at face value
   ============================================================ */
...
function DropCard({ drop, loading }) {
  if (loading || !drop) {
    return (
    ...
    <div className="card card-pad dropcard">
      ...
      <div className="drop-row">
        <div className="drop-stat">
          ...
          <div className="v">{FV.fmtMoney(drop.faceValue)}</div>
        </div>
...
function Fan() {
  ...
  const [result, setResult] = useState(null); // PurchaseResp
  ...
  function buy() {
    ...
    setResult(null);
    FV.api.purchase({
      dropId: drop.id, agentId: "25459", chain: "ethereum", humanId: nullifier,
    }).then((res) => {
      setBuying(false);
      setResult(res.data);
      // refresh remaining
              ...
              {!verified ? (
                ...
                <React.Fragment>
                  ...
                  <div className="id-readout">
                    ...
                    <span className="h">{FV.shortHash(nullifier)}</span>
                  </div>
                </React.Fragment>
              )}
            </div>
                ...
                <div className={"result " + (approved ? "approved" : "denied")} data-testid="purchase-result">
                  ...
                  {approved && result.ticketId ? (
                    <div className="result-ticket">
                      <FaceValueChip amount={result.faceValue} />
...
ReactDOM.createRoot(document.getElementById("root")).render(<Fan />);
