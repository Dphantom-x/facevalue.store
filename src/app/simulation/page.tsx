...
import { FAN_AGENT, SCALPER_AGENT } from "@/lib/format";
...
function shuffledRanks(n: number): number[] {
  ...
  order.forEach((tileIdx, step) => {
    rank[tileIdx] = step;
  });
  return rank;
}

function makeTargetsA(): string[] {
...
function Grid({ treatment, targets, rank, fill }: { treatment: string; targets: string[]; rank: number[]; fill: number }) {
  return (
...
function VerdictCard({ kind, tag, state, trust }: { kind: string; tag: string; state: string; trust: Trust | null }) {
...
export default function SimulationPage() {
  ...
  const [targetsA, setTargetsA] = useState<string[]>(() => makeTargetsA());
  const [rankA, setRankA] = useState<number[]>(() => shuffledRanks(TOTAL));
  const targetsB = useMemo<string[]>(() => Array.from({ length: TOTAL }, () => "fan"), []);
  const rankB = useMemo<number[]>(() => Array.from({ length: TOTAL }, (_, i) => i), []);

  const [fanCard, setFanCard] = useState("idle");
  const [fanTrust, setFanTrust] = useState<Trust | null>(null);
  const [scalperCard, setScalperCard] = useState("idle");
  const [scalperTrust, setScalperTrust] = useState<Trust | null>(null);
  ...
  function run() {
    ...
    api.trustCheck(FAN_AGENT.agentId, FAN_AGENT.chain).then((res) => {
      const t = res && res.data && res.data.trust;
      if (t) { setFanTrust(t); setFanCard("result"); } else setFanCard("empty");
    }).catch(() => setFanCard("empty"));
    api.trustCheck(SCALPER_AGENT.agentId, SCALPER_AGENT.chain).then((res) => {
      const t = res && res.data && res.data.trust;
